...
* Gradient: is not a valid solution. As some linear gradients could work, as the gradient is constant. However, Neural Networks use an activation function that changes the output, such as ReLU, to a non-linear function causes sensitivity violation. Since the gradient of ReLU does not directly show the influence of the input to the output. A ReLU
